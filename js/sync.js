...
window.Sync = (() => {
  ...
  let activeSpace = null;   // 当前查看/连接所用的共享空间 gistId（来自 ?space= 或本机同步）

  function setActiveSpace(id) { activeSpace = id || null; }
  ...
  async function push(force) {
    ...
    try {
      ...
      status('正在加密上传…');
      // 已开启密码锁且本机已解锁：用「密码派生密钥」加密共享数据，
      // 这样任何拿到链接的人都能用密码解密，无需你的 Token。
      const body = (Lock && Lock.hasKey())
        ? await sealWithKey(DB.data, Lock.dataCryptoKey())
        : await seal(DB.data, s.token);
  ...
  // 发布「全局锁」公开元数据：任何人（含未设同步的好友）无需 Token 即可读取，
  // 仅含 enabled/salt/verifier（verifier 是 MAGIC 的密文，不含任何明文）。
  async function pushLockFile(meta) {
    const s = cfg();
    if (!s.token || !s.gistId) throw new Error('no gist');
    if (!secureCtx()) throw new Error('insecure');
    const res = await fetch(apiBase() + '/' + s.gistId, {
      method: 'PATCH',
      headers: authHeaders(),
      body: JSON.stringify(withBody({ files: { 'yiyunge-lock.json': { content: JSON.stringify(meta) } } }))
    });
    if (!res.ok) throw new Error('lockfile ' + res.status);
    return true;
  }

  // 无鉴权读取公开 Gist 的锁元数据（好友仅需链接中的 space 参数即可）
  async function fetchLockMetaPublic(gistId) {
    if (!gistId) return null;
    try {
      const res = await fetch(apiBase() + '/' + gistId);
      if (!res.ok) return null;
      const j = await res.json();
      const c = j.files && j.files['yiyunge-lock.json'] && j.files['yiyunge-lock.json'].content;
      if (!c) return null;
      return JSON.parse(c);
    } catch (e) { return null; }
  }

  // 无鉴权读取公开 Gist 的加密数据（好友无需 Token；解密需要密码）
  async function readPublicData(gistId) {
    const res = await fetch(apiBase() + '/' + gistId);
    if (!res.ok) return null;
    const j = await res.json();
    return j.files && j.files[FNAME] && j.files[FNAME].content;
  }

  async function pull() {
    const s = cfg();
    // 情况 A：已输入密码（memKey 存在）→ 直接解密公开 Gist 的共享数据，无需 Token
    if (Lock && Lock.hasKey()) {
      const gid = activeSpace || s.gistId;
      if (!gid) { status('未连接共享空间', 'warn'); throw new Error('no gist'); }
      busy = true;
      try {
        status('正在下载共享内容…');
        const content = await readPublicData(gid);
        if (!content) throw new Error('云端暂无数据');
        const data = await openWithKey(content, Lock.dataCryptoKey());
        DB.importJSON(JSON.stringify(data));
        s.baseSavedAt = DB.data.__savedAt || Date.now();
        s.lastSync = Date.now();
        DB.save();
  ...
  // 读取「全局锁」状态：优先读公开的锁元数据文件（无需 Token，也不依赖数据加密方式）
  async function fetchLockMeta() {
    const s = cfg();
    if (!s.gistId) return null;
    if (!secureCtx()) return null;
    try {
      const res = await fetch(withToken(apiBase() + '/' + s.gistId), { headers: authHeaders() });
      if (!res.ok) return null;
      const j = await res.json();
      ...
      // 兼容旧版：从解密后的数据里取 __lockMeta
      if (!s.token) return null;
      const content = j.files && j.files[FNAME] && j.files[FNAME].content;
      if (!content) return null;
      const data = await readRemote(content, s.token);
      return (data && data.__lockMeta) || null;
    } catch (e) { return null; }
  }

  return { cfg, schedulePush, push, pull, ensureGist, status, refreshStatus, fetchLockMeta,
           fetchLockMetaPublic, pushLockFile, readPublicData, setActiveSpace,
