  ...
  function moduleTodayTodo(ctx) {
    return `
        <div class="card">
          ...
          <div class="todo-list">
            ${ctx.undone.length ? ctx.undone.slice(0, 6).map(x => {
      ...
      return `<div class="todo-item" data-p="${x.priority}" data-tid="${x.id}">
                ...
                <div class="t-body"><div class="t-title">${U.esc(x.title)}</div>
                  ...
                  ${x.autoKey ? '<span class="tag gold">自动</span>' : ''}</div></div>
              </div>`;
    }).join('') : `<div class="empty" style="padding:22px"><p>今日待办已清空</p></div>`}
          </div>
          ${ctx.undone.length > 6 ? `<button class="btn btn-ghost btn-sm" style="width:100%;justify-content:center;margin-top:9px" data-go="todo">查看全部 ${ctx.undone.length} 条</button>` : ''}
        </div>`;
  }
    ...
    U.rebind(root, 'dash', e => {
      ...
      if (!b) {
        ...
        case 'backup': {
          ...
          U.toast(name ? `已导出「${name}」，请到浏览器「下载」里查看` : '请先解锁应用再导出备份', name ? 'ok' : 'warn');
          break;
        }
