...
const DB = (() => {
  ...
  const LOCK_KEY = 'sakura-desk-lock';   // 与 lock.js 共用，避免锁开启时误读明文

  // 锁开启时本地不存明文，先返回空白壳，等解锁后由 Lock 调 unlockData 填充
  function lockEnabledRaw() {
    try { const r = localStorage.getItem(LOCK_KEY); if (r) { const c = JSON.parse(r); return c.enabled === true; } } catch (e) {}
    return false;
  }
  ...
  function demoData() {
    ...
    const students = [
      mk(normDateFlexible('240815') || U.addDays(t, -30), '李妈妈', '李明', 'tc_wang', '妈妈很关注细节，每节课后一定要文字反馈；孩子基础中等偏上，函数薄弱。', 'active',
        [{ grade: '高二', subject: '数学', tuition: 300, commission: 50, duration: 90 }]),
      mk(normDateFlexible('250302') || U.addDays(t, -20), '张爸爸', '张昊', 'tc_chen', '爸爸做生意比较忙，微信回复慢；中考目标 110+。', 'active',
        [{ grade: '初三', subject: '英语', tuition: 240, commission: 60, duration: 120 },
         { grade: '初三', subject: '数学', tuition: 260, commission: 55, duration: 120 }]),
      mk(normDateFlexible('250520') || U.addDays(t, -10), '王妈妈', '王思', 'tc_wang', '冲刺阶段，要求老师准时；一个孩子在这上语数英三科。', 'active',
        [{ grade: '高三', subject: '物理', tuition: 380, commission: 80, duration: 120 },
         ...
         { grade: '高三', subject: '英语', tuition: 300, commission: 60, duration: 120 }]),
      mk(normDateFlexible('250610') || U.addDays(t, -5), '刘妈妈', '刘洋', 'tc_me', '孩子注意力短，建议 60 分钟一节；家长在意性价比。', 'trial',
        [{ grade: '五年级', subject: '数学', tuition: 200, commission: 40, duration: 60 }], { trialDate: U.addDays(t, -1) })
    ];
  ...
  function load() {
    // 已开启应用锁：先不载入真实数据，等解锁后再填充（避免明文出现在内存）
    if (lockEnabledRaw()) { const b = blank(); b.__locked = true; return b; }
    try {
  ...
  function save() {
    try {
      ...
      if (typeof Lock !== 'undefined' && Lock.isEnabled() && Lock.hasKey()) {
        Lock.encryptInto(data);              // 锁开启且已解锁：写密文(ENC_KEY)
  ...
  // 解锁成功后由 Lock 调用：用解密出的对象替换内存数据
  function unlockData(obj) {
    if (!obj || typeof obj !== 'object') return;
    const b = blank();
    for (const k in b) if (obj[k] === undefined) obj[k] = b[k];
    obj.settings = Object.assign(b.settings, obj.settings || {});
    migrateSubjects(obj);
    data = obj;
  }
  ...
  // 返回 null 表示应用锁未解锁，此时不应导出（避免产生空白备份）
  function buildSnapshot() {
    if (data.__locked) return null;
    const now = new Date();
  ...
  // 导出当前全部数据为 JSON 文件。返回文件名；若应用锁未解锁则返回 null（避免导出空文件）
  function exportJSON() {
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('storage', e => {
      // 加密模式下忽略跨标签页同步（避免把密文当明文解析）
      if (typeof Lock !== 'undefined' && Lock.isEnabled()) return;
  ...
  return {
    ...
    touch, setRemoteHandler, unlockData,
