...
const Changelog = (() => {
  ...
  const CHANGELOG = [
    { ver: `v2.11.0`, date: `2026-09-12`, held: false, divider: false,
      items: [`<b>模板库新增「上午 / 下午 / 不选」归属</b>：新建或编辑每日模板时多了一个「归属」下拉——选「上午」或「下午」就<b>强制</b>该模板生成的事项固定进对应栏；选「不选（按时间自动）」则按模板里填的时间以中午 12 点为界自动归栏（没填时间默认上午）。模板还可选填一个时间，用于「不选」时决定归栏、也作为事项本身的发生时间。主页「导入模板」与提醒事项页「导入全部」都按这套规则统一归栏。`,
        `<b>修：上午拖到下午不再跳回页面顶端</b>：此前两栏之间约有 8px 缝隙，手指拖到缝隙时落点判定返回空、事项卡在旧（上午）栏、看着像「跳到最上面」，得再往下拖一次。现改为落点取<b>中线最近的列表</b>，跨栏移动丝滑跟手，落下后 slot 直接钉死为目标栏并写库。`,
        `<b>过午自动收起上午</b>：今天一过中午 12:00、且当天开了「上下午分栏」时，<b>上午栏整块隐藏</b>，上午里<b>未完成</b>的事项会自动挪到下午最前面（已完成的随上午栏一起隐藏）。这一搬迁是<b>持久化</b>的——刷新、切到别的设备都保持下午最前，不会回弹。`] },
    { ver: `v2.10.0`, date: `2026-09-12`, held: false, divider: false,
      items: [`<b>提醒事项支持「上午 / 下午」分栏</b>：每天日期行最右侧新增一个「上下午分栏」勾选框，<b>默认每天都分</b>；哪天不想分，取消那一天的勾即可（各天互不影响）。开启后当天事项按<b>中午 12 点</b>自动归入上午或下午，没设时间的事项默认归上午、也可以手动拖到下午；模板库导入同样按 12 点自动归栏。<b>拖动范围也扩大了</b>：现在能<b>在栏内调顺序、跨栏移动、甚至拖到别的某一天</b>——跨天会改这条事项的归属日期，跨栏只是改它属于上午还是下午，<b>你原本设置的时间一概不动</b>。日程可在当天内跨栏 / 调顺序（同样不改时间），暂不支持跨天改期。`] },
    { ver: `v2.9.0`, date: `2026-09-12`, held: false, divider: false,
      items: [`<b>修「删掉的待办又冒出来」的老毛病</b>：重复模板生成的待办（如秋招投递）此前只有<b>提醒事项页</b>会清理「已删除的那天」，日历 / 主页不清理，于是出现「主页 → 日历又看到它，点一下提醒事项再回日历又没了」这种来回反复。现在把清理提到<b>进入任何页面之前</b>统一执行，各页面状态始终一致，删掉就是真删掉了。<b>拖拽排序不再卡顿</b>：原来手指每移动一点就重新测量所有行的位置（强制同步布局），还用 left / top 移动拖拽影子、每帧重排整页；现改为每帧最多处理一次、行位置缓存复用、影子用 transform 走合成层，拖动明显更跟手。<b>点行任意处即可打勾</b>：不必再精准点那个小方框，点标题 / 备注 / 空白都能标记完成或取消完成；编辑、删除、拖拽手柄各自区域不受影响（日程行仍保持「点行 = 编辑日程」，因为它没有完成态）。<b>模板库两个按钮靠右聚拢</b>：「批量删除 ✓」与「新增 ＋」此前被均分到卡片两端，现并成一组顶到最右、挨在一起。`] },
    { ver: `v2.8.5`, date: `2026-09-11`, held: false, divider: false,
      items: [`<b>提醒事项操作键靠拢：编辑 / 删除按钮贴住最右拖拽手柄</b>——上版把拖拽手柄移到行尾最右后，编辑 / 删除按钮与它中间隔着约 18px 空隙，看着散。现把拖拽手柄的左外边距内收，使「编辑 ✎ 删除 🗑 拖拽 ⠿」三者紧贴成一组、整体靠右贴边，操作更顺手、不再各占一边。`] },
    { ver: `v2.8.4`, date: `2026-09-11`, held: false, divider: false,
      items: [`<b>排序交互优化：取消「上移 / 下移」箭头，拖拽手柄移到行尾最右</b>——之前每条右侧有编辑 / 删除 + 上移下移四个图标，挤且慢。现彻底去掉上移 / 下移按钮，<b>拖拽手柄（⠿）移到每行最右边</b>，做成独立的圆角胶囊：平时半透明低调，手指 / 鼠标悬停或抓取时变粉色并浮出浅粉底，比之前更清楚、更好按。按住它直接拖动即可排序，勾选 / 编辑 / 删除按钮照旧在它左边，互不干扰。桌面提醒事项页与手机端 7 日视图均生效。`] },
    { ver: `v2.8.3`, date: `2026-09-11`, held: false, divider: false,
      items: [`<b>提醒事项支持拖拽排序（鼠标 + 触屏通用）</b>——之前只能反复点「上移 / 下移」调顺序，慢。现每行左侧加一个 <code>⠿</code> 拖拽手柄，按住拖动即可把当天待办 / 日程直接拖到目标位置，比点箭头快得多。只有手柄响应拖动，<b>勾选框、编辑 / 删除按钮、页面滚动都不受影响</b>；落下后按最终顺序重写当天每条记录的 <code>order</code> 并刷新修改时间（跨端同步逻辑不变）。上移 / 下移按钮保留作备用。「已完成」区和遗留卡片不参与拖拽。手机端 7 日视图同样可用。`] },
    { ver: `v2.8.2`, date: `2026-09-11`, held: false, divider: false,
      items: [`<b>修了两个模板待办的跨端 bug（排序跳顶 / 主页显示两条重复的秋招投递）</b>——根因是：模板库生成的待办此前<b>每台设备各自随机编号</b>（桌面端 <code>td_xxx</code>、移动端 <code>tpli_模板ID_日期</code>），两份编号不同，云同步时被判成两条独立记录，于是主页有时冒出 2 条同名的「秋招投递」、刷新只是偶发去重才消失；而「排序下移」其实是在两条孪生待办之间换来换去、始终停在顶部，看着就像「一下跳到最上面」。现彻底统一：①所有设备启动期都跑一遍 <code>normalizeTemplateTodoIds()</code>，把带 <code>tplId</code> 的待办归一为确定性编号 <code>tpli_模板ID_日期</code> 并按编号去重合并（保留较新一条）；②模板导入（待办页「导入模板」与主页「导入模板」）直接生成这个确定性编号并双重去重，不再产生随机编号。手动添加的待办不受影响。改动只动自动生成的模板待办编号，不碰你手写的任何数据。`] },
    { ver: `v2.8.1`, date: `2026-09-10`, held: false, divider: false,
      items: [`<b>导出日历：支持勾选哪些日程</b>——之前点「导出日历」只能全导或全不导。现改为先弹一个勾选清单：默认全选，可逐条取消；顶部还能按<b>分类</b>下拉筛选、按<b>标题搜索</b>，并一键「全选 / 全不选 / 反选」；确认后只导出勾选的日程。清单里每条都标了分类、下次发生日和重复次数。待办依旧不导出。`] },
    { ver: `v2.8.0`, date: `2026-09-01`, held: false, divider: false,
      items: [`<b>新功能：日程一键导出到系统日历（.ics）</b>——日历页右上角新增「导出日历」按钮，点击后把逸云阁里的<b>日程</b>导出成标准 <code>.ics</code> 文件，可导入手机自带日历（iOS 日历 / Android Google 日历），由系统原生提醒（关 App、锁屏也响），相当于白嫖系统日历的提醒能力。导出规则：只导出<b>今天起未来 365 天</b>内发生的日程、过去的不导出；重复日程按每天展开成独立事件；每条日程的「提醒」设置（事件发生时 / 5·15·30 分钟前 / 1 小时前）会一并转成系统日历的提醒，选「无」则不提醒；全天事件、跨天事件、地点、备注、分类颜色都正确带过去。零后端、纯本地生成，不碰你的任何数据。提醒事项（待办）按你之前的选择本次不导出，只导出日程。`] },
    { ver: `v2.7.7`, date: `2026-09-10`, held: false, divider: false,
      items: [`<b>课程完成：标记后立刻同步</b>——在「标记完成」和「编辑保存」两处都加了一道显式 <code>Sync.push()</code>，不再只等 20 秒自动拉取去抖。之前课程完成走的和待办是同一套同步管道、代码上没漏写时间戳，但自动拉取偶发跳过会让人误以为「不同步」，现在标记完几乎即时就推上去了。`,
        `<b>模板待办：排序 / 打勾现在能跨端同步了</b>——模板库生成的待办原本每台设备各自随机编号，手机排的序、打的勾平板对不上号。现改为按「模板 + 日期」固定生成确定性编号（<code>tpli_模板ID_日期</code>），多端生成的是同一条记录；已存在的旧随机编号会在打开时自动迁移成新规则，极少数同模板同天重复项合并保留较新的一条。手动添加的待办不受影响。改动只动自动生成的模板待办编号，不涉及你手写的任何数据。`] },
    { ver: `v2.7.6`, date: `2026-09-10`, held: false, divider: false,
      items: [`<b>手机端：每天「已完成(n)」折叠头与上方横线之间的留白收紧</b>——先前横线（分隔条）上方距最后一条事项约 20px、下方距「已完成」标题约 22px，空得发虚。现把当天分组里的分隔条上下边距由 <code>14px</code> 收到 <code>4px</code>、归档头上下内边距由 <code>8px/8px</code> 收到 <code>4px/3px</code>，横线紧贴最后一条事项，标题也紧贴横线。改动只作用于手机端当天分组（<code>.day-group</code>），平板 / 桌面端的「提醒事项」页面布局完全不变。`] },
    { ver: `v2.7.5`, date: `2026-09-10`, held: false, divider: false,
      items: [`<b>跨端同步：打勾与排序现在能同步到别的设备了</b>——同步规则是「谁新听谁的」（按 <code>_mt</code> 修改时间），但此前<b>打勾</b>和<b>手动排序</b>这两个操作压根没有记录修改时间，另一台设备下载时就会判定自己那份旧数据更新、把完成状态和排序打回原样。现已补上：打勾 / 取消打勾、上移 / 下移排序都会刷新时间戳（排序会同时给待办和日程打上）。`,
        `<b>日历：提醒事项的顺序与「提醒事项」页面一致</b>——日历里的待办原先只按「时间 → 优先级」排，完全没用手动排的 <code>order</code>。现改为与提醒事项页同一套排序口径（🚩置顶 → 手动顺序 → 时间 → 优先级），在提醒事项页调过的顺序，日历里完全一样。日历仍保持<b>日程在上、提醒事项在下</b>分两组显示。`,
        `<b>手机端：已完成的提醒事项自动收进折叠区</b>——以往手机端打勾后仍一直摊在列表里，现与平板一致：每天下方出现「已完成(n)」，默认收起，点一下才展开。因为手机端每天各有一个折叠区，其内部查找方式也从「按 id」改为「按位置定位」（否则只会开关第一个）。`,
        `<b>平板 / 桌面端：提醒事项行高压到约 28px</b>——内边距由 8px 收到 4px，勾选框 20px 保持不变，整行比方框略高一点。手机端不受影响，仍保持 8px 的宽松触控区。`,
        `<b>已知待办</b>：模板库生成的提醒事项，目前是每台设备各自生成、id 互不相同，所以手机上给它排序后平板不会跟着变（手动添加的事项则能同步）。彻底修好需要把这类事项的 id 改成按「模板 + 日期」固定生成，因涉及改写已有数据，待确认后再动。`] },
