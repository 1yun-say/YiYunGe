...
const Changelog = (() => {
  function render() {
    ...
    <div class="card">
      ...
      <ul class="log-list">
        <li><b>全身体检（第二批）：逐文件 + 跨模块 + 双盲审计，共修复 17 项</b>：① <b>重复任务跨视图一致</b>——周视图勾选重复实例改用被点单元格真实日期（此前误记成周焦点，导致跨视图错位）；重复完成态在日历/待办/首页统一读 completedDates 并显示绿色勾选。② <b>遗留列表不再误收重复任务</b>——重复任务只在其发生日出现，不再永久塞进「遗留」。③ <b>「拉到今天」不再偏移重复锚点</b>——仅移动非重复过期任务。④ <b>本地数据损坏不再静默覆盖</b>——损坏时保留原串并尝试从内嵌备份恢复，否则明确提示而非用示例数据覆盖真实数据。⑤ <b>ServiceWorker 离线首开崩溃修复</b>——预缓存键剥离 ?v 查询串，装到主屏离线打开不再把 index.html 当 JS 返回。⑥ <b>删老师级联解绑课时</b>，避免财务按老师分组出现幽灵项；⑦ 年重复以 2/29 为起点时非闰年规整为 2/28；⑧ 自动同步冲突后停止轮询，避免每 20s 重复弹警告；⑨ 删除单节课加确认；⑩ 首页/小助手写死白色背景改用主题变量（夜览不再白块）；⑪ 补全 favicon；⑫ 保存学员时对 Todo/App 做存在性守护等若干稳健性修复。</li>
