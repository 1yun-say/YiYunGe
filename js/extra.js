...
const AI = (() => {
  ...
  function reply(text) {
    ...
    if (/(学员|学生|档案|家长)/.test(q)) {
      return `现在有 ${DB.data.students.length} 位学员档案。可直接逐项填写家长称呼、年级学科、课时费、抽成等；也支持用标准编码一键拆解（默认格式：日期-年级学科-家长-抽成/课时费|时长），例如 240815-高二数学-李妈妈-50/300|90。`;
    }
